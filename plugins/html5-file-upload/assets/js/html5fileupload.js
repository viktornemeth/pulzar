...
(function (window, $, undefined) {
	...
	$.html5fileupload = function html5fileupload(options, element) {
		...
		//buttons
		this.button				= {}
		this.button.edit		= '<div class="button tiny info edit"><i class="fa fa-pencil"></i></div>'
		//this.button.saving		= '<div class="btn btn-warning saving">Saving... <i class="glyphicon glyphicon-time"></i></div>';
		
		this.button.start		= '<div class="button tiny success start"><i class="fa fa-play"></i></div>';
		this.button.restart		= '<div class="button tiny success restart"><i class="fa fa-repeat"></i></div>';
		this.button.cancel		= '<div class="button tiny alert cancel"><i class="fa fa-ban"></i></div>';
		this.button.reset		= '<div class="button tiny info reset"><i class="fa fa-refresh"></i></div>';
		this.button.remove		= '<div class="button tiny alert remove"><i class="fa fa-times"></i></div>';
		this.button.done		= '<div class="button tiny success done"><i class="fa fa-check"></i></div>';
		this.button.del			= '<div class="button tiny alert del"><i class="fa fa-trash"></i></div>';
		this.button.download	= '<a class="button tiny info download"><i class="fa fa-download"></i></a>';
		
		this.button.allstart	= '<div class="button tiny success allstart"><i class="fa fa-play-circle-o"></i> ' + (this.options.labelAllStart || '') + '</div>';
		this.button.allstop		= '<div class="button tiny info allstop"><i class="fa fa-dot-circle-o"></i> ' + (this.options.labelAllStop|| '') + '</div>';
		this.button.alldone		= '<div class="button tiny info alldone"><i class="fa fa-check-circle"></i> ' + (this.options.labelAllDone|| '') + '</div>';
		this.button.allremove	= '<div class="button tiny alert allremove"><i class="fa fa-times-circle"></i> ' + (this.options.labelAllRemove|| '') + '</div>';
		
		this.progressbar		= '<div class="progress"><span class="meter"></span></div>'
	...
	$.html5fileupload.prototype = {
		...
		start: function($file, file) {
			...
			$(progressbar).find('.meter').css('width','0%');
         ...
         var obj_id = $('#obj_id').val();
			reader.onloadend = function(evt) {
				var ajax	= $.ajax({
					...
					type: 		options.ajaxType || 'POST',
					cache: 		false,
					data: 		{ name: $file.data('filename'), filename: $file.data('file'), file: evt.target.result, data: options.data, command:command, secure:secure, obj_id:obj_id},
					headers:	options.ajaxHeaders,
					xhr: function() {
					    var xhr = new window.XMLHttpRequest();
					    xhr.upload.addEventListener("progress", function(evt){
					      if (evt.lengthComputable) {
					        $(progressbar).find('.meter').css('width',Math.round((evt.loaded / evt.total) * 100) + "%");
					      }
					    }, false);
					    return xhr;
					},
					...
					if (response.result == true) {
						$(progressbar).find('.meter').css('width','100%');
